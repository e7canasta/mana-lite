...
impl ScanInstant {
    ...
    const fn from_instant(v:Instant)->Self{Self(v)}
...
#[derive(Debug,Clone)] pub struct ControlPolicy { pub person_class:ClassName,pub presence_enabled:bool,pub data_stale_ms:u64,pub scan_period_ms:u64,pub face_dwell_roi:Option<[u32;4]>,pub person_detection_roi:Option<[u32;4]>,pub face_edge_margin_px:u32 }
pub struct ControlState { pub loop_id:LoopId,pub tracker:Option<Tracker>,pub presence:PresenceFilter,pub occupancy:OccupancyStateMachine,pub zone_engine:Option<ZoneEngine>,pub fsm_engine:Option<FsmEngine>,pub health:Health,pub fsm_context:FsmSceneContext,pub last_scan_at:Instant,pub scan_seq:u64,pub policy:ControlPolicy }
...
#[derive(Debug,Clone)] pub struct ScanTimeline{loop_id:LoopId,start:Instant,period_ms:u64,tick:u64}impl ScanTimeline{pub fn new(loop_id:LoopId,start:Instant,period_ms:u64)->Self{Self{loop_id,start,period_ms:period_ms.max(1),tick:0}}pub fn loop_id(&self)->&LoopId{&self.loop_id}pub fn now(&self)->ScanInstant{ScanInstant::from_instant(self.start+Duration::from_millis(self.tick*self.period_ms))}pub fn advance(&mut self)->ScanInstant{self.tick+=1;self.now()}}
...
mod tests {
    ...
    fn scan_marks_signal_invalid_when_evidence_is_stale() {
        ...
        let mut timeline =
            ScanTimeline::new(LoopId::default_loop(), start, data_stale_ms + 1);
        timeline.advance();
        let events = scan(&mut state, &image, &timeline);
