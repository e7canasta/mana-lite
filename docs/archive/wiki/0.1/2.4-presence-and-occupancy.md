...
The filter accumulates real elapsed time (milliseconds) across processed keyframes, not frame counts. Each `update()` receives the pipeline's measured `dt_ms` and adds it to the `positive_ms` / `empty_ms` accumulators; `signal_valid = false` time does not count. This keeps the hysteresis constant when the camera degrades (e.g., stalled to 1 fps) instead of stretching it.

- **On ms**: The accumulated real time a person must be detected before the state becomes `PresenceState::Present` [src/presence.rs114-116](src/presence.rs#L114-L116)
- **Off ms**: The accumulated real time a person must be missing before the state becomes `PresenceState::Absent` [src/presence.rs147-150](src/presence.rs#L147-L150)
- **Signal Holding**: If the state is `Present` but a frame is missing detections, the filter "holds" the last known observation while `empty_ms < off_ms`. This prevents the tracker from losing continuity during momentary occlusions or inference failures [src/presence.rs131-144](src/presence.rs#L131-L144)
...
|**Filter**|`PresenceFilter`|Tracks `positive_ms` / `empty_ms` accumulators and the `last_person` observation [src/presence.rs30-38](src/presence.rs#L30-L38)|
...
The `OccupancyStateMachine` (OSM) consumes the filtered presence signals and raw track counts to determine the `RoomCardinality`. Like the presence filter, the OSM is time-based: it uses `std::time::Instant` to apply real-time millisecond hysteresis [src/occupancy.rs56-64](src/occupancy.rs#L56-L64)
...
sequenceDiagram
    ...
    DE->>PF: update(observations, signal_valid, dt_ms)
    Note right of PF: Applies on_ms / off_ms<br/>[src/presence.rs:114]
