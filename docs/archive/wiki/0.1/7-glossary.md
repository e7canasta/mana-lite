...
**Sources:** [src/main.rs65-94](src/main.rs#L65-L94) [src/fsm.rs49-55](src/fsm.rs#L49-L55) [src/occupancy.rs56-64](src/occupancy.rs#L56-L64) [src/track.rs93-100](src/track.rs#L93-L100) [src/infer.rs95-97](src/infer.rs#L95-L97)
...
- **Code Pointer:** `BlueprintConfig` in [src/cascade.rs6](src/cascade.rs#L6-L6) and [src/main.rs133-149](src/main.rs#L133-L149)
...
- **Code Pointer:** `CascadeScheduler` in [src/cascade.rs122-126](src/cascade.rs#L122-L126) `CascadeRule` in [src/cascade.rs6](src/cascade.rs#L6-L6)
- **Logic:** Rules define requirements like `requires_min_confidence` or `requires_region` [src/cascade.rs105-114](src/cascade.rs#L105-L114)
...
- **Code Pointer:** `CompactMask` in [std/mana-geometry/src/compact_mask.rs65-69](std/mana-geometry/src/compact_mask.rs#L65-L69)
- **Implementation:** Uses `vernier-mask` for RLE encoding [std/mana-geometry/src/compact_mask.rs106](std/mana-geometry/src/compact_mask.rs#L106-L106)
...
- **Code Pointer:** `dwell_timers` in [src/fsm.rs53](src/fsm.rs#L53-L53)
- **Logic:** Tracked via `Instant` and compared against `dwell_ms` in configuration [src/fsm.rs81-101](src/fsm.rs#L81-L101)

### FSM (Finite State Machine)

The top-level logic engine that determines the semantic state of the room (e.g., `in_bed`, `exiting`, `searching`).

- **Code Pointer:** `FsmEngine` in [src/fsm.rs49-55](src/fsm.rs#L49-L55)
- **Logic:** Evaluates `FsmGuard` conditions against `FsmSceneContext` [src/fsm.rs119-142](src/fsm.rs#L119-L142)
...
- **Zone Hysteresis:** `hysteresis_ms` in `ZoneState` [src/zones.rs30](src/zones.rs#L30-L30)
- **Occupancy Hysteresis:** Timers for `single_confirm_ms` and `empty_confirm_ms` [src/occupancy.rs195-196](src/occupancy.rs#L195-L196)
...
- **Code Pointer:** `Kalman7` in [src/track.rs17](src/track.rs#L17-L17) and [src/kalman.rs](src/kalman.rs)

### Occupancy Cardinality

The high-level classification of how many people are in a room.

- **Variants:** `Empty`, `Single`, `Multiple` [src/occupancy.rs5-9](src/occupancy.rs#L5-L9)
- **Logic:** Managed by `OccupancyStateMachine` [src/occupancy.rs56-64](src/occupancy.rs#L56-L64)
...
- **Code Pointer:** `PresenceFilter` in [src/presence.rs30-38](src/presence.rs#L30-L38)
- **Logic:** Transitions between `Absent`, `Present`, and `Ambiguous` [src/presence.rs5-9](src/presence.rs#L5-L9)
...
- **Code Pointer:** `CropRect` in [src/infer.rs17-22](src/infer.rs#L17-L22)

### Track

A persistent identity assigned to a sequence of detections across multiple frames.

- **Code Pointer:** `Track` in [src/track.rs10-23](src/track.rs#L10-L23)
- **Lifecycle:** Transitions through `Created`, `Updated`, `Lost`, and `Deleted` [src/track.rs26-46](src/track.rs#L26-L46)

### Zone

A named spatial region within the frame (e.g., "bed", "door") used to trigger FSM guards based on track intersection.

- **Code Pointer:** `ZoneEngine` in [src/zones.rs36-38](src/zones.rs#L36-L38)
- **Logic:** Evaluates if a `Track` intersects a `ZoneState` rectangle [src/zones.rs64-73](src/zones.rs#L64-L73)
...
```
**Sources:** [src/main.rs65-94](src/main.rs#L65-L94) [src/fsm.rs119-142](src/fsm.rs#L119-L142) [src/presence.rs55-59](src/presence.rs#L55-L59) [src/track.rs142-145](src/track.rs#L142-L145) [src/zones.rs64-65](src/zones.rs#L64-L65)
...
|**Hungarian Algorithm**|[src/assignment.rs](src/assignment.rs)|Used by the `Tracker` to solve the linear assignment problem between existing tracks and new detections.|
...
|**JSONL**|[src/logger.rs](src/logger.rs)|The structured logging format (JSON Lines) used for events and metrics.|
|**NMS**|[src/infer.rs89](src/infer.rs#L89-L89)|Non-Maximum Suppression. A post-processing step to remove redundant overlapping bounding boxes.|
|**POI**|[src/presence.rs33](src/presence.rs#L33-L33)|Person of Interest. The primary entity the pipeline is configured to track.|
|**Rerun**|[src/viz.rs](src/viz.rs)|The external visualization tool used for real-time debugging of the pipeline state.|
|**Static ROI**|[src/infer.rs107](src/infer.rs#L107-L107)|A fixed rectangular crop defined in `models.toml` that the `InferEngine` applies before running a model.|

**Sources:** [src/main.rs48-51](src/main.rs#L48-L51) [src/track.rs3](src/track.rs#L3-L3) [src/infer.rs89-90](src/infer.rs#L89-L90) [src/presence.rs33](src/presence.rs#L33-L33) [src/logger.rs37](src/logger.rs#L37-L37)
