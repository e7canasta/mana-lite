...
>
> - **El bloque por modelo tiene una clave nueva, `gated`.** Distingue "el
>   estado del FSM no pidió este modelo" de "la regla lo salteó" (`skips`), que
>   llevan a lugares opuestos del sistema. Es aditivo: no cambia el significado
>   de ninguna clave existente, así que `JSONL_SCHEMA_VERSION` sigue en 2.
>
> - **`min_ms` y `max_ms` ya no salen cuando el modelo no tuvo llamadas.** El
>   acumulador de mínimo arranca en `u64::MAX` y se emitía dividido por mil:
>   `"min_ms":18446744073709551` publicado como si fuera una latencia. Sin
>   llamadas no hay mínimo, y ausencia no es cero.
>
