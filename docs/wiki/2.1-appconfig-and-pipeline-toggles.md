...
|`image_max_res`|`u32`|`0`|Longest side allowed on the wire; the integer subsampling factor is derived from it. `0` sends native resolution|
...
The two combine. Prefer `image_format = "jpeg"` with `image_max_res = 0`: overlays are logged in native pixel coordinates, so keeping the image's dimensions means nothing needs compensating. An unknown `image_format` falls back to `raw` with a warning rather than refusing to boot — visualization is a debugging aid, not a boot requirement. See [Visualization](6-visualization-\(rerun-integration\)) for the measured payload table.
