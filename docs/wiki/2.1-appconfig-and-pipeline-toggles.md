...
> página:
>
> - **`tentative_max_age_ms` por defecto pasó de 600 ms a 6000 ms**, en `TrackerConfig::default()` y en `src/config/app.rs`. El valor viejo era más corto que un intervalo de keyframe: con la configuración por defecto ningún track llegaba a su segunda medición y ninguno se confirmaba.
>
...
|`dedup_max_suppress_ms`|`u64`|How long an identical keyframe may be suppressed before it is emitted anyway. Default 5000.|

#### Keyframe deduplication has an expiry

Consecutive keyframes with an identical 64-bit digest are suppressed to avoid reprocessing an image that did not change. That suppression **expires**, and it must.

On a completely still scene an encoder can emit byte-identical IDRs indefinitely. Without an expiry the suppression would never end, the pipeline would go blind, and — worst of all — **the suppression itself would drive the FSM to `data_stale`**. Upstream there would be no way to tell "the scene did not change" from "the stream died", which for a clinical system is not an acceptable ambiguity: `data_stale` means "I lost the signal" and has to keep meaning that.

Because of that coupling, `dedup_max_suppress_ms` must stay below `[health] data_stale_ms`. Bootstrap rejects the inconsistent combination rather than accepting a knob that would prevent nothing.
...
### 2b. Visualization Transport (`[viz]`)

`VizConfig` governs the Rerun bridge. Beyond `enabled` and `rerun_addr`, two orthogonal knobs control what actually crosses the wire — relevant because an uncompressed 1080p frame is 6.2 MB and the bridge is the heaviest consumer of the pipeline thread's slack.

|Field|Type|Default|Purpose|
|---|---|---|---|
|`enabled`|`bool`|`false`|Wire the bridge at all|
|`rerun_addr`|`String`|`0.0.0.0:9876`|Viewer endpoint|
|`image_format`|`String`|`"raw"`|`"raw"` or `"jpeg"`. `"jpeg"` cuts payload ~32× **without** changing resolution|
|`image_quality`|`u8`|`75`|JPEG quality, clamped to 1-100|

`"jpeg"` is the only lever offered here, deliberately: it reduces the wire payload ~32× while keeping native resolution, so overlays logged in native pixel coordinates need no compensation. Downscaling was considered and rejected — it shrinks the image while overlays stay in full-resolution coordinates, trading correctness for less benefit than compression already delivers. An unknown `image_format` falls back to `raw` with a warning rather than refusing to boot — visualization is a debugging aid, not a boot requirement. See [Visualization](6-visualization-\(rerun-integration\)) for the measured payload table.

**Sources:** [src/config/app.rs63-100](src/config/app.rs#L63-L100) [src/viz/mod.rs](src/viz/mod.rs)
...
- [2b. Visualization Transport](2.1-appconfig-and-pipeline-toggles#2b-visualization-transport-viz)
