...
- [](std/mana-rtsp/Cargo.toml)
- [](std/mana-types/Cargo.toml)
- [](std/mana-types/src/lib.rs)
- [](std/mana-video/Cargo.toml)
- [](std/mana-viz/Cargo.toml)
...
flowchart TB
    ...
    MG --> MT
```
**Sources:** [std/mana-video/Cargo.toml8](std/mana-video/Cargo.toml#L8-L8) [std/mana-viz/Cargo.toml8](std/mana-viz/Cargo.toml#L8-L8) [std/mana-geometry/Cargo.toml1-13](std/mana-geometry/Cargo.toml#L1-L13)
...
- **`PixelFormat`**: An enum defining supported image formats like `Rgb8`, `Nv12`, and `Yuyv`. It includes utility functions for calculating buffer sizes: `frame_byte_size(width, height)` [std/mana-types/src/lib.rs3-50](std/mana-types/src/lib.rs#L3-L50)
- **`RawFrameV1`**: The primary container for decoded video frames, including metadata like `frame_id`, `timestamp_ns`, and `capture_mono_ns` for latency tracking [std/mana-types/src/lib.rs55-65](std/mana-types/src/lib.rs#L55-L65)
- **`DetectionBatchV1`**: A fixed-size array (`MAX_DETECTIONS = 128`) of `DetectionV1` objects, used to pass inference results between the `InferEngine` and the `Tracker` [std/mana-types/src/lib.rs85-106](std/mana-types/src/lib.rs#L85-L106)
- **`SceneMsgV1`**: The final output of the vision pipeline, containing a list of `SceneEntityV1` (tracked objects) and `ZoneV1` (spatial regions) [std/mana-types/src/lib.rs239-254](std/mana-types/src/lib.rs#L239-L254)

**Sources:** [std/mana-types/src/lib.rs1-287](std/mana-types/src/lib.rs#L1-L287)
...
- **`CompactMask`**: Uses Run-Length Encoding (RLE) via the `vernier-mask` crate to store segmentation masks efficiently [std/mana-geometry/Cargo.toml5-9](std/mana-geometry/Cargo.toml#L5-L9)
...
**Sources:** [std/mana-geometry/Cargo.toml1-16](std/mana-geometry/Cargo.toml#L1-L16)
...
- **FFmpeg Integration**: Wraps `ffmpeg-next` to handle H.264/H.265 decoding [std/mana-video/Cargo.toml11](std/mana-video/Cargo.toml#L11-L11)

**Sources:** [std/mana-video/Cargo.toml1-12](std/mana-video/Cargo.toml#L1-L12)
...
- **`mana-rtsp`**: Contains utilities for handling H.264 Annex-B streams. It is responsible for parsing NAL units (Network Abstraction Layer) from RTSP packets before they are sent to the decoder [std/mana-rtsp/Cargo.toml1-5](std/mana-rtsp/Cargo.toml#L1-L5)
- **`mana-viz`**: A bridge to the `Rerun.io` visualization SDK. It converts `mana-types` structures into Rerun archetypes (e.g., `Boxes2D`, `Image`, `Tensor`) for real-time debugging [std/mana-viz/Cargo.toml5-9](std/mana-viz/Cargo.toml#L5-L9)

**Sources:** [std/mana-rtsp/Cargo.toml1-8](std/mana-rtsp/Cargo.toml#L1-L8) [std/mana-viz/Cargo.toml1-12](std/mana-viz/Cargo.toml#L1-L12)
...
**Sources:** [std/mana-types/src/lib.rs55-65](std/mana-types/src/lib.rs#L55-L65) [std/mana-types/src/lib.rs239-254](std/mana-types/src/lib.rs#L239-L254) [std/mana-video/Cargo.toml5](std/mana-video/Cargo.toml#L5-L5) [std/mana-viz/Cargo.toml5](std/mana-viz/Cargo.toml#L5-L5)
