...
use basic::{
    write_evidence_age_event, write_frame_event, write_health_event, write_meta_event,
    write_metrics_event, write_scan_deadline_event,
};
use control::{
...
use detection::{
    write_body_parts_event, write_consolidated_detection_event, write_cross_model_validation_event,
    write_depth_event, write_depth_region_event, write_detection_event,
};
...
pub fn write_event(event: &Event, ts: &str, buf: &mut Vec<u8>) {
    ...
    match event {
        ...
        Event::CrossModelValidation { .. } => write_cross_model_validation_event(event, buf),
        Event::BodyParts { .. } => write_body_parts_event(event, buf),
