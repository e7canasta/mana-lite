...
use super::event::{DetRecord, Event, FaceDwellTimerRecord, MaskRecord};
use crate::metrics::{PerClassFrameStats, PerModelMetrics};
...
fn write_meta_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Meta {
        event,
        detail,
        attrs,
...
fn write_health_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Health {
        event,
        frame_id,
        cycle_us,
...
fn write_frame_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Frame {
        frame_id,
        is_keyframe,
        decode_ms,
...
fn write_detection_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Detection {
        frame_id,
        model,
        infer_ms,
...
fn write_det_bbox(d: &DetRecord, buf: &mut Vec<u8>) {
...
fn write_det_mask(mask: &MaskRecord, buf: &mut Vec<u8>) {
...
fn write_detection_per_class(per_class: &Option<PerClassFrameStats>, buf: &mut Vec<u8>) {
...
fn write_depth_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Depth {
        version,
        frame_id,
        model,
...
fn write_depth_region_event(event: &Event, buf: &mut Vec<u8>) {
...
fn write_consolidated_detection_event(event: &Event, buf: &mut Vec<u8>) {
...
fn write_entity_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Entity {
        track_id,
        class,
        bbox,
...
fn write_zone_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Zone {
        zone,
        event,
        class,
...
fn write_fsm_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Fsm {
        from,
        from_label,
        to,
...
fn write_presence_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::Presence {
        scan_seq,
        evidence_frame_id,
        observations_age_ms,
...
fn write_face_dwell_event(event: &Event, buf: &mut Vec<u8>) {
    let Event::FaceDwell {
        scan_seq,
        evidence_frame_id,
        observations_age_ms,
...
fn write_face_dwell_timers(active_timers: &[FaceDwellTimerRecord], buf: &mut Vec<u8>) {
...
fn write_metrics_event(event: &Event, buf: &mut Vec<u8>) {
...
fn write_metrics_models(model_metrics: &HashMap<String, PerModelMetrics>, buf: &mut Vec<u8>) {
